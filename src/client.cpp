...
int get_variant(int min_number, int max_number) {
    int variant;
    string str;
    cout << "Please select from "<< min_number <<" to "<< max_number <<" and press enter\n";
    getline(cin, str);
    while(sscanf(str.c_str(),"%d", &variant) != 1  || variant < min_number || variant > max_number ){
...
void draw_header(){
    printf("\e[1;1H\e[2J");
    cout << "\n";
    char header[6][55] = {"\t _____ _       _____            _____", 
                          "\t|_   _(_)     |_   _|          |_   _|", 
...
void draw_game( vector<vector<char> > &board, int check_progress){
    ...
    } else if(check_progress == 0) cout << "Please wait for the other player's turn..." << endl;
    else if(check_progress == -1) cout << "Congratulations! You won!\n" << endl;
    else if(check_progress == -2) cout << "Sorry! You loose!\n" << endl;
    else cout << "You have a draw!\n" << endl;
}
...
void chat_message(){
    ...
    Packet packettype = P_ExitToMenu;
    send(socketClient, (char*)&packettype, sizeof(Packet), 0);
}
...
int client(int argc, char* argv[]) {
    ...
    if(argc != 3) return error("Launch error\nPlease write in the format: ./client --connect \"IP Number:Port Number\"");
        ...
        if (to_string(atoi(cpy_PORT.c_str())) != cpy_PORT || atoi(cpy_PORT.c_str()) < 1024 || atoi(cpy_PORT.c_str()) > 65535) {
