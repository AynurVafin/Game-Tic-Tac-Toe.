...
v3 board(all_boards, vector<vector<char> >(size_board, vector<char>(size_board)));
...
bool check_draw(int session) {
    for (int i = 0; i < size_board; i++)
    {
        for (int j = 0; j < size_board; j++)
        {
            if(board[session][i][j] != 'X' && board[session][i][j] != 'O') return false;
        }
    }
    return true;
}
...
bool check_win(int session) {
    if(board[session][0][0] == board[session][1][1] && board[session][0][0] == board[session][2][2]) {
        win_combin[0] = 1;
        win_combin[1] = 5;
        win_combin[2] = 9;
        return true;
    } 
    if(board[session][0][2] == board[session][1][1] && board[session][0][2] == board[session][2][0]) {
        win_combin[0] = 3;
        win_combin[1] = 5;
        win_combin[2] = 7;
        return true;
    } 
    if(board[session][0][0] == board[session][0][1] && board[session][0][0] == board[session][0][2]) {
        win_combin[0] = 1;
        win_combin[1] = 2;
        win_combin[2] = 3;
        return true;
    } 
    if(board[session][1][0] == board[session][1][1] && board[session][1][0] == board[session][1][2]) {
        win_combin[0] = 4;
        win_combin[1] = 5;
        win_combin[2] = 6;
        return true;
    } 
    if(board[session][2][0] == board[session][2][1] && board[session][2][0] == board[session][2][2]) {
        win_combin[0] = 7;
        win_combin[1] = 8;
        win_combin[2] = 9;
        return true;
    } 
    if(board[session][0][0] == board[session][1][0] && board[session][0][0] == board[session][2][0]) {
        win_combin[0] = 1;
        win_combin[1] = 4;
        win_combin[2] = 7;
        return true;
    } 
     if(board[session][0][1] == board[session][1][1] && board[session][0][1] == board[session][2][1]) {
        win_combin[0] = 2;
        win_combin[1] = 5;
        win_combin[2] = 8;
        return true;
    } 
     if(board[session][0][2] == board[session][1][2] && board[session][0][2] == board[session][2][2]) {
        win_combin[0] = 3;
        win_combin[1] = 6;
        win_combin[2] = 9;
        return true;
...
void push_board(int index, int session){
    for (int i = 0; i < size_board; i++)
    {
        send(Connections[index], board[session][i].data(), sizeof(char)*size_board, 0);
    }
}
...
bool ProcessPacket(int index, Packet packettype) {
    ...
    case P_Check:{
        ...
        if(!checker) break;
        cout << "S" << session+1 << "P" << index+1<<"-" << variant+1 << endl;
    ...
    case P_PlayGame:{
        ...
        if(ready_game.size() == 1) {
            ...
            sessions[session].push_back(index);
             cout << "S" << session+1 << "P" << index+1 << "-connection game" << endl;
            push_msg("The second player connected!\nStarting the game...\nYou are playing crosses (X). Good luck!", session, 0);
            ...
            cout << "S" << sess_now+1 << "P" << index+1 << "-connection game" << endl;
            sessions[sess_now].push_back(index);
            push_msg("You are the first player!\nPlease wait for the other player to connect...", sess_now, 0);
        }
    } break;
    case P_Message:
    {
...
int server(int argc, char* argv[]) {
    ...
    if(argc != 3) return error("Launch error, please write in the format: ./server --port \"Port Number\"");
    else if(strcmp(argv[1] , "--port") != 0) {
        cout << "./server: unrecognized option \""<< argv[1] << "\"" << endl;
        return error("usage: ./server --port");
    } else if (to_string(atoi(argv[2])) != argv[2]  || atoi(argv[2]) < 1024  || atoi(argv[2]) > 65535) {
        return error("Invalid port format, please enter a port between 1024 and 65535");
    }
    PORT = atoi(argv[2]);
    ...
    int optval = 1;
    setsockopt(socketServer, SOL_SOCKET, SO_REUSEPORT | SO_REUSEADDR, &optval, sizeof(optval));
    if(socketServer == -1) {
    ...
    cout << "Listening for incoming connections on port "<< PORT << "..." << endl; // ljqwdwql
    
    int newConnection;
    for (int i = 0; i < 100; i++)
    {
        newConnection = accept(socketServer, (struct sockaddr*) &saddr, (socklen_t*)&sizeof_saddr);
        if(newConnection < 0 ){
            string err = "Error connection player/fan number " + to_string(CounterClient+1);
            return error(err.c_str());
        } else {
            cout << "Player " << to_string(CounterClient+1) << " connected..." << endl;
            Packet packet = P_Menu;
